function step4b_firstlevel_glm_task_sS(participant_id,HomeDir)

% this function creates a firstlevel GLM of task fMRI blocks
% using SPM12
...
OutDir=fullfile(HomeDir,participant_id,'firstlevel_glm_sS');
if exist(OutDir)==0
    mkdir(fullfile(HomeDir,participant_id),'firstlevel_glm_sS')
else
...
for iBlock=1:length(Blocks) %Loop over runs
   ...
   %% get logfile data
   [TrialData]=sub_get_auditory_events(participant_id,iBlock,HomeDir);
   ...
   %% Get nii files
    flist{iBlock} = spm_select('ExtFPList',DataDir,['^swu',participant_id,'_','b', ...
