function step2b_MVPA_task_crossnobis_xclass_cv_b(Sample,HomeDir,roi_name,glm_name)

% This function uses a leave-one-run-out (LORO) cross-validation design to 
% compute the crossnobis distance between the trials with (da vs ba) responses
...
for iSubj=1:length(Sample)
    ...
    idx_reg_stimuli=find((contains(regressor_names(1,:),'Unamb') | ...
        contains(regressor_names(1,:),'Amb')) & ...
    ...
    cfg.files.xclass(contains(reg_stimuli(1,:),'Unamb'))=1;
    ...
    cfg.results.dir = fullfile(HomeDir,participant_id,'MVPA_task_crossnobis_xclass_cv_b');
