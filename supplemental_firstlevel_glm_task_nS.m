function supplemental_firstlevel_glm_task_nS(participant_id,HomeDir)
% 
% this function computes a firstlevel GLM for each stimulus (high vs low F3  
% response (ga vs da) combination 
% using SPM12
...
OutDir=fullfile(HomeDir,participant_id,'supplemental_firstlevel_glm_task_nS');
if exist(OutDir)==0
    mkdir(fullfile(HomeDir,participant_id),'supplemental_firstlevel_glm_task_nS')
else
...
for iBlock=1:length(Blocks) %Loop over runs
   ...
   Stimuli={'Amb','Unamb'};%unique(TrialData.Stimulus_name);
   ...
   %% identify trials n trials (/ga/ amb, /da/ amb, /ga/ unamb, /da/ unamb)
   Cnt=1;
   for i=1:length(Stimuli)
       for j=1:length(Response)
           names{Cnt}=[Stimuli{i} '_' upper(Response{j})];
           if strcmp(Stimuli{i},'Amb')==1
               TrialNr{Cnt}=find(contains(TrialData.Stimulus_name,lower(Stimuli{i})) & ...
                   contains(TrialData.Response,Response{j}));
           elseif strcmp(Stimuli{i},'Unamb')==1
               TrialNr{Cnt}=find((contains(TrialData.Stimulus_name,'da') | ...
                   contains(TrialData.Stimulus_name,'ga')) & ...
   ...
   %% balance number of trials with /ga/ and /da/ responses 
   
   TrialData.Response_recoded=TrialData.Response; % Created additional colume in TrialData. This columne will be recoded  
   
   rng(iBlock)
   
   for i=1:length(Stimuli)
       idx_Stim_GA=find(strcmp(names,[Stimuli{i} '_' 'GA']));Stim_GA_trials=TrialNr{1,idx_Stim_GA};n_Stim_GA=numel(Stim_GA_trials);
       idx_Stim_DA=find(strcmp(names,[Stimuli{i} '_' 'DA']));Stim_DA_trials=TrialNr{1,idx_Stim_DA};n_Stim_DA=numel(Stim_DA_trials);
       
       if  n_Stim_GA > n_Stim_DA
           Stim_GA_trials_new=Stim_GA_trials(sort(randsample(n_Stim_GA,n_Stim_DA)));
           NotSelected=setdiff(Stim_GA_trials,Stim_GA_trials_new); 
       elseif n_Stim_GA < n_Stim_DA
           Stim_DA_trials_new=Stim_DA_trials(sort(randsample(n_Stim_DA,n_Stim_GA)));
           NotSelected=setdiff(Stim_DA_trials,Stim_DA_trials_new);
       end
       % not selected trials are set to NaN responses
       TrialData.Response_recoded(NotSelected)={'NaN'};
   end
   ...
   for i=1:length(Stimuli)
       for j=1:length(Response)
           names{Cnt}=[Stimuli{i} '_' upper(Response{j})];
           if strcmp(Stimuli{i},'Amb')==1
               onsets{Cnt}=TrialData.Stimulus_onset(contains(TrialData.Stimulus_name,lower(Stimuli{i})) & ...
                   contains(TrialData.Response_recoded,Response{j}));
           elseif strcmp(Stimuli{i},'Unamb')==1
               onsets{Cnt}=TrialData.Stimulus_onset((contains(TrialData.Stimulus_name,'da') | ...
                   contains(TrialData.Stimulus_name,'ga')) & ...
                   contains(TrialData.Response_recoded,Response{j})  & TrialData.ShamRamp==0);
           end
           Cnt=Cnt+1;
       end
   end
...
Stimuli={'Amb_DA','Amb_GA','Unamb_DA','Unamb_GA'};
